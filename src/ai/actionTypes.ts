...
export const ACTION_TYPES = [
  ...
  'similar', 'pockets', 'annotations', 'palette', 'ligand', 'densityfit',
  'ramachandran',
] as const;
...
export const ACTION_REFERENCE: { type: ActionType; value: string; note: string }[] = [
  ...
  { type: 'ligand', value: '"" for every ligand, or "HEM"', note: 'What a bound ligand touches, grouped by the residue touching it, with the kind of each contact — hydrogen bond, ionic, aromatic stacking, van der Waals — and the closest approach. Returns a selection covering the ligand and its whole site, ready for a component action. Every criterion is geometric: report it as proximity, never as affinity or binding strength, and never infer a mechanism from a long list. Two things it deliberately withholds — histidine is not counted as charged because its protonation state is not in the file, and a residue bonded to the ligand is reported as covalently linked rather than as a partner, because a modified residue is not a bound one. Contacts are found only within the deposited coordinates, so a ligand at a crystal contact has partners that are not listed.' },
  { type: 'ramachandran', value: '"" or "top 5"', note: 'Backbone phi/psi for every residue in the pane, classified against reference contours measured from 900 high-resolution structures. Returns the percentage favoured, the outlier count and a ranked list of outliers with a ready-made selection. Computed here from the coordinates, so unlike the validation action it works on a local file or a predicted model, and it covers residues wwPDB never scored. It agrees with wwPDB\'s own outlier percentage to about 0.05 points. Note that MolView drops atoms deposited at zero occupancy, so an entry that models unobserved backbone that way (1KX5 does) will report fewer outliers here than wwPDB does — the residues are absent rather than forgiven.' },
  { type: 'densityfit', value: 'null', note: 'Real-space correlation per residue between the model and the density map currently shown, so it requires the density action first. Reports the ligands — which wwPDB publishes no per-residue score for, and which are usually what a reader doubts — and the worst-fitting residues. The number is NOT on wwPDB\'s scale: the public-data benchmark has mean per-entry r = 0.624 on four calibration entries and 0.426 on five additional entries, with one case at 0.094. Do not quote it against a published threshold or call a value "good". Even rankings require inspecting the map and other evidence; negative correlation does not identify a cause.' },
  { type: 'palette', value: '"saturation 0.6" | "intensity 1.3" | "reset"', note: 'Adjust how strong the pane\'s colours are, without changing the colour scheme itself. 1 is the scheme as authored, 0 saturation is greyscale, above 1 pushes further from grey. Panes start at saturation 2, intensity 1, and "reset" returns there rather than to 1/1. Range 0 to 3. Raising intensity is the risky one: the image clamps at full brightness with no rolloff, so past about 1.2 the bright end of any colour flattens. Applies to every representation and to a molecular surface, but never to a density map, whose colours identify which map and which sign of the difference, and it is ignored entirely on a pane coloured by a measurement (B-factor, pLDDT, RSRZ, outliers, hydrophobicity, AlphaMissense, N-to-C rainbow, or a Coulombic surface) or using the colour-blind-safe palette. Saying "reset" is the way to undo it; do not report a change to those panes as having taken effect.' },
  ...
  { type: 'interfaces', value: '"A,B" for one pair, "A" for one chain, "" for all, or "top 5"', note: 'Find which chains touch which, by heavy-atom contact and by the area each pair buries (Shrake-Rupley SASA, reported as interface area — half the total change, the convention PISA uses), including symmetry copies when an assembly is shown and the neighbours of the crystal lattice. Partners are labelled: an unlabelled one is in the deposited coordinates, "assembly copy N" is part of the biological unit, and "crystal neighbour" is only a lattice contact. That label plus the area is how packing is told from biology — a crystal contact typically buries a few hundred A2 and a genuine interface considerably more — so say which you think it is and show the numbers rather than asserting it. Use it instead of reasoning from stoichiometry about what packs against what. Naming a pair also returns a ready-made selection covering the contact residues on both sides: pass that string to a component action to draw the interface itself. There is no distance operator in the selection grammar, so this is the only way to select an interface.' },
