...
export const ACTION_REFERENCE: { type: ActionType; value: string; note: string }[] = [
  ...
  { type: 'densityfit', value: 'null', note: 'Real-space correlation per residue between the model and the density map currently shown, so it requires the density action first. Reports the ligands — which wwPDB publishes no per-residue score for, and which are usually what a reader doubts — and the worst-fitting residues. The number is NOT on wwPDB\'s scale: it agrees with their per-residue RSCC at about r = 0.62 and reads roughly 0.2 lower, so compare residues within the one structure and never quote it against a published threshold or call a value "good". Negative correlation is the only absolute statement available: the density is somewhere the model is not.' },
