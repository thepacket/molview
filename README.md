...
**Crystal packing, and telling it from biology.** The assembly answers what the
depositor believes; the lattice answers what the crystal actually did. They are
different sets, and only one of them used to be reachable — ubiquitin generates
a single assembly copy, itself, and reported no contacts at all while sitting
...
independently from the space group and cell, with no assembly at all, it buries
1,614 Å².
...
biological ones both exist — and a viewer that prints "biological" in green
would be believed more than it deserves.
